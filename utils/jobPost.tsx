      ...
      docRefProfiles
        ...
        .then(async (doc) => {
          if (doc.exists) {
            ...
            let newPostRef = firebase.firestore().collection("jobsPosts").doc();
            ...
            await newPostRef
              .set({
                ...
                jobLocation,
                educationLevel,
                salaryRange: [Number(minSalary), Number(maxSalary)],
              ...
              .then(async () => {
                ...
                await firebase
                  ...
                  .update({
                    jobsPosts: firebase.firestore.FieldValue.arrayUnion(
                      newPostRef.id
                    ),
                  });
              })
...
export const EditJobPost = async ({
  ...
  const postRef = firebase.firestore().collection("jobsPosts").doc(postId);
...
export const deleteJobPost = (post: JobPost) => {
  ...
  postRef
    ...
    .then(async (doc: firebase.firestore.DocumentSnapshot) => {
      if (doc.exists) {
        ...
        userRef
          .update({
            jobsPosts: firebase.firestore.FieldValue.arrayRemove(post.postId),
          })
          .then(() => {
            console.log(
...
export const deleteJobComment = async (post: JobPost, commentId: string) => {
  const unsubscribe = firebase.auth().onAuthStateChanged(async (user) => {
    if (user) {
      const docRefProfiles = firebase
        .firestore()
      ...
      docRefProfiles
        ...
        .then(async (doc) => {
          if (doc.exists) {
            ...
            // Remove the comment reference from the jobsPosts document
            await firebase
              .firestore()
              .collection("jobsPosts")
              .doc(post.postId)
